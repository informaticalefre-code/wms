    ...
    ***************************************************************************/
    function handleBarcode(scanned_barcode){
        const ModalPacking   = document.getElementById("PackingModal").style.display;
        console.log("scanned_barcode"+scanned_barcode);
        if (ModalPacking == 'block'){
            /* Si la ventana modal de packing está abierta verificamos que el código escaneado
            ...
            if (ModalPacking == "" || ModalPacking == "none"){
                let encontrado  = false;
                let id_producto ;
                for(var i in global_packing_productos){
                    if (Object.values(global_packing_productos[i]).indexOf(scanned_barcode) >= 0){
